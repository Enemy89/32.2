...
struct Film {
    std::string filmName;  // Добавлено поле для названия фильма
    std::string country;
...
// Функция для создания JSON-документа с информацией о фильме
...
int main() {
    // Создание пяти различных фильмов
    ...
    // Создание JSON-документа с информацией о пяти фильмах
    nlohmann::json database;
    for (const auto& movie : movies) {
        database[movie.filmName] = createJsonDocument(movie);
    }
    ...
    // Поиск актера по имени и вывод информации о фильмах
    std::string actorName;
    std::cout<<"Enter name actor for find: "<<std::endl;
    std::cin>>actorName;
    for (const auto& movie : movies) {
        if (movie.characters.find(actorName) != std::string::npos) {
            std::cout << "Film: " << movie.filmName << ", Role: " << movie.characters << std::endl;
        }
    }
    file.close();
}